...
def main() -> None:
    ...
    require(index, ">市政</span>", "dedicated civic navigation")
    require(index, ">さがす</span>", "search center-nav label")
    require(index, "aria-label=\"直方の情報をさがす\"", "search accessible name")
    require(index, "./ui-v2.js?v=21", "versioned citizen-first runtime")
    ...
    require(index, "./ui-home-v4.js?v=23", "event-led home runtime")
    require(home_ui, "地域・ごみ収集を設定", "home settings label")
    forbid(home_ui, "よく見る地域：", "removed personalized region label")
    ...
    require(index, "./civic-portal.js?v=4", "people-first civic portal runtime")
    ...
    require(civic_portal, 'if (nav === "civic")', "civic bottom navigation handler")
    require(civic_portal, 'state.politicsSection = "people";', "people-first civic destination")
    require(civic_portal, '["politics", "people", "civic", "works", "nearby", "money"]', "persistent civic routes")

    require(css, "min-height: 44px", "minimum interactive target")
    require(css, "@media (max-width: 520px)", "single-column mobile breakpoint")
    require(sw, "mytown-civic-v35-home-copy", "service-worker cache revision")
