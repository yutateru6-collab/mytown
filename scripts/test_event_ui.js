...
assert.equal((home.match(/class="v4-home-event-card"/g) || []).length, 2, "home must show exactly two event cards");
const homeSources = [...home.matchAll(/class="v4-home-event-source">([^<]+)</g)].map((match) => match[1]);
assert.equal(homeSources.length, 2, "every home event must show its source type");
assert.equal(new Set(homeSources).size, 2, "the two home events must come from different publishers");
