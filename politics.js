...
function politicsDataBanner() {
  ...
  if (state.politicsLoadError) {
    return `<div class="sync-banner is-warning"><strong>市長・市議会の情報を読み込めませんでした</strong><span>市のページで確認できます。</span></div>`;
  }
...
function politicsHome() {
  const p = state.politics;
  if (state.politicsLoading) return `<section class="page"><div class="hero"><p class="eyebrow">市長・議会</p><h1>市長・市議会と、決まり方</h1><p>公式資料を読み込んでいます。</p></div><div class="card info-card"><div class="loading-line"></div><div class="loading-line short"></div></div></section>`;
  const mayor = p.mayor || {};
  ...
  return `<section class="page politics-page">
    <div class="hero politics-hero"><p class="eyebrow">🏛 市長・議会</p><h1>市長・市議会と、決まり方</h1><p>市長や市議会のことを、初めてでも分かるように説明します。</p></div>
    ${politicsDataBanner()}
    ${guideBubble("市長は何をする人？ 市議会には誰がいる？ というところから見られます。難しい言葉もその場で説明します。")}
    ...
    <div class="card beginner-card">
      <span class="beginner-kicker">まず知っておきたい</span>
      ...
      ${guideBubble("2027年1月でも『2026年度』と書かれることがあります。年度は4月から次の3月までだからです。", "正式には、2026年度＝2026年4月1日〜2027年3月31日")}
      <button class="text-button" type="button" data-politics-section="glossary">ほかの難しい言葉も見る →</button>
    </div>

    ${state.data?.council ? `<div class="section"><div class="section-head"><div><h2>次の市議会</h2><p>次はいつ、何をする？</p></div></div>${councilPreview(state.data.council)}</div>` : ""}
    ...
    <div class="card election-teaser">
      <div><span class="pill verified">市の資料で確認</span><h2>次の直方市長選・市議選</h2></div>
      <p><strong>市長の任期が終わる日：</strong>${esc(electionMayor.termEnd || "確認中")}</p>
      <p><strong>市議の任期が終わる日：</strong>${esc(electionCouncil.termEnd || "確認中")}</p>
      <p class="muted">投票日は、現在の市のページではまだ発表されていません。決まり次第更新します。</p>
      ${guideBubble("任期が終わる日と、選挙の投票日は同じとは限りません。")}
...
function membersView() {
  ...
  return `<section class="page politics-page">${politicsBackButton()}<div class="hero"><p class="eyebrow">直方市議会</p><h1>議員19人、全員を見る</h1><p>所属委員会と、2026年6月に届け出た質問テーマを確認できます。</p></div>
    ${guideBubble("名前を知らなくても大丈夫です。関心のある質問テーマから見てください。")}
    <div class="card info-card"><div class="split-head"><div><span class="pill verified">市の議員名簿</span><h2>議長 ${esc(c.chair || "")}</h2><p>副議長 ${esc(c.viceChair || "")}</p></div><strong>${Number(c.seats || members.length)}人</strong></div><p class="muted">名簿の更新日：${esc(c.membersSourceUpdated || "")}</p>${sourceLink(c.membersSourceUrl, "市の議員名簿を見る")}</div>
    <div class="member-list">${members.map((m) => `<button class="member-card" type="button" data-politician="${esc(m.name)}"><div><span class="pill ${m.role !== "議員" ? "verified" : ""}">${esc(m.role)}</span><h3>${esc(m.name)}</h3><p>${esc(m.committee || "委員会情報なし")}</p></div><div class="member-meta">${m.recentQuestions?.length ? `<span>${m.recentQuestions.length}件<br><small>6月の質問通告</small></span>` : `<span>—<br><small>通告一覧に記載なし</small></span>`}<b aria-hidden="true">›</b></div></button>`).join("")}</div>
    <div class="card info-card"><h2>この一覧に質問が載っていない場合</h2><p>ここに表示しているのは、2026年6月議会の一般質問通告一覧だけです。議員の活動全体を示すものではありません。</p>${guideBubble("一つの資料だけで、議員の活動全体を評価しません。")}</div>
  </section>`;
}
...
function electionsView() {
  ...
  return `<section class="page politics-page">${politicsBackButton()}<div class="hero"><p class="eyebrow">🗳️ 選挙</p><h1>次の市長選・市議選は？</h1><p>任期が終わる日と、まだ発表されていない投票日を分けて表示します。</p></div>
...
function glossaryView() {
  ...
  return `<section class="page politics-page">${politicsBackButton()}<div class="hero"><p class="eyebrow">📚 役所・議会ことば図鑑</p><h1>「それ何？」から始める</h1><p>難しい言葉を、普通の言葉から説明します。</p></div>
...
answerFor = function answerForWithPolitics(question) {
  ...
  if (/市長/.test(q) && /誰|だれ|名前|氏名|現在|今の|任期|何期/.test(q) && p.mayor) {
    return `<div class="card answer-card" role="status"><span class="pill verified">市の資料で確認できました</span><h3>今の直方市長は ${esc(p.mayor.name)}</h3><p>${esc(p.mayor.term || "任期情報を確認中です")}。</p><button class="text-button" type="button" data-tab="politics" data-open-politics="mayor">市長ページで詳しく見る →</button></div>`;
  }
  if (/市長/.test(q)) {
    return `<div class="card answer-card" role="status"><span class="pill">市の資料では分かりませんでした</span><h3>その市長に関する質問は、現在の公開資料から確認できません</h3><p>質問に合わない基本情報を、答えの代わりには表示しません。</p><button class="text-button" type="button" data-tab="politics" data-open-politics="mayor">参考として市長の基本情報を見る →</button></div>`;
  }
