...
if (typeof currentSelectedItem === "function" && typeof combinedSearchItems === "function") {
  currentSelectedItem = function currentSelectedItemFromAllSyncedData() {
    return combinedSearchItems().find((item) => item.id === state.selectedId)
      || (state.data?.communityEvents?.events || []).find((item) => item.id === state.selectedId)
      || null;
  };
}
...
if (typeof syncBanner === "function") {
  syncBanner = function syncBannerWithSourceHealth() {
    ...
    return `<div class="sync-banner" role="status"><div><span class="official-badge">市の公開情報から</span><strong>直方市の情報を掲載</strong></div><span>最終確認：${generatedAt ? esc(formatDateTime(generatedAt)) : esc(verifiedOn || "確認済み")}</span></div>`;
  };
}
